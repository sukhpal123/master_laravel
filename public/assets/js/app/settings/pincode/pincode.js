...
$("#edit_gram").change(function() {
  ...
  var dtPincode = $('#dtPincode').DataTable({
    "paging": true,
    "lengthChange": true,
    "searching": true,
    "ordering": true,
    ...
    "columnDefs": [{ targets: [0], className: 'dt-center valign dt-w3' },
                  { targets: [1,2,3,4,5], className: 'dt-left valign dt-w15' },
                  {targets: [6], className: 'dt-center dt-w17 valign'}],
    dom: 'Bfrtip',
    "ajax": "AppController/dtPincode",
    "order": [] 
});//Datatable View

// console.log(dtPincode);
if ($.fn.DataTable.isDataTable('#dtPincode')) {
    var dataTable = $('#dtPincode').DataTable();
}
    ...
    $("#addLinkageForm").unbind('submit').bind('submit', function() {
      ...
      var pincode = $("#pincode").val();
        ...
        if(pincode == "") {
          ...
          $("#pincode").after('<p class="text-danger error-text">Pincode is required</p>');
        } else {
          $("#pincode").closest('.form-group').removeClass('has-error');
          $("#pincode").closest('.form-group').addClass('has-success');                   
        }
      if (state && district && taluk && gram && village && pincode) {
          ...
          success:function(response) {
            ...
            if(response.success == true) {
              $(function() {
                ...
                Toast.fire({
                  icon: 'success',
                  title: 'Pincode added successfully!'
                });
              });
              // reset the form
              $("#addLinkageForm")[0].reset();
              $('#dtPincode').DataTable().ajax.reload();
            } else {
      ...
      $.ajax({
        url: 'AppController/getPincode',
          ...
          success:function(response) {
            ...
            $('#edit_village').val(response.v_id).trigger("change");
            $('#edit_village').data("id", response.v_id);

            $('#p_id').val(response.p_id);
            $('#edit_pincode').val(response.p_code);
            $(".editModal").append('<input type="hidden" name="p_id" id="p_id" value="'+response.p_id+'"/>');
            $("#editLinkageForm").unbind('submit').bind('submit', function() {
            $(".text-danger").remove();
            var form = $(this);
            var edit_pincode = $("#edit_pincode").val();
            if(edit_pincode === ''){
             $("#edit_pincode").closest('.form-group').addClass('has-error');
               $("#edit_pincode").after('<p class="text-danger error-text">Pincode is required</p>');
             } else {
               $("#edit_pincode").closest('.form-group').removeClass('has-error');
               $("#edit_pincode").closest('.form-group').addClass('has-success');                   
             }   
           ...
           if (edit_pincode) {
            $.ajax({
                ...
                if(response.success == true) {
                    ...
                    Toast.fire({
                        icon: 'success',
                        title: 'Pincode info updated successfully'
                    });
                    });
                    $('#dtPincode').DataTable().ajax.reload();
                } else {
