$(document).ready(function(){
	var dtDesigtype = $('#dtDesigtype').DataTable({
      // "paging": true,
      // "lengthChange": true,
      "processing": true,
      "serverSide": true,
  ...
  $("#addIdBtn").on('click', function(e) {    
    // reset the form 
    $("#adddesigForm")[0].reset();
    // remove the error 
    $(".form-group").removeClass('has-error').removeClass('has-success');
    $(".text-danger").remove();
    $("#adddesigForm").unbind('submit').bind('submit', function() {
      $(".text-danger").remove();
      var form = $(this);
      // validation                 
      var designation = $("#designation").val();
        ...
        if(designation == "") {
                ...
                $("#designation").after('<p class="text-danger error-text">Designation is required</p>');
        } else {
                $("#designation").closest('.form-group').removeClass('has-error');
        ...
        $.ajax({
          ...
          success:function(response) {
            ...
            if(response.success == true) {
              ...
              // reset the form
              $("#adddesigForm")[0].reset();
              $('#dtDesigtype').DataTable().ajax.reload();
            } else {
                Toast.fire({
                  icon: 'error',
  ...
  Swal.fire({
    ...
    preConfirm: function() {
       return new Promise(function(resolve) {                
          $.ajax({
          url: 'AppController/delDesignation',
          type: 'POST',
...
function editMember(id = null) {
    if(id) {
      ...
      $.ajax({
        url: 'AppController/getDesignation',
          type: 'post',
          data: {member_id : id},
          dataType: 'json',
          success:function(response) {
            ...
            $("#editDesigForm").unbind('submit').bind('submit', function() {
            $(".text-danger").remove();
            var form = $(this);
            // validation
            ...
            if(edit_designation == "") {
                $("#edit_designation").closest('.form-group').addClass('has-error');
                $("#edit_designation").after('<p class="text-danger">Designation is required</p>');
            } else {
                $("#edit_designation").closest('.form-group').removeClass('has-error');
                $("#edit_designation").closest('.form-group').addClass('has-success');                   
            }
            ...
            $.ajax({
                ...
                if(response.success == true) {
                    ...
                    const Toast = Swal.mixin({
                        toast: true,
                        position: 'top',
                        target:"#editDesigForm",
                        showConfirmButton: false,
                        timer: 3000
                    });
                    Toast.fire({
                        icon: 'success',
                        title: 'Designation info updated successfully'
                    });
                    });
                    $('#dtDesigtype').DataTable().ajax.reload();
                } else {
